...
interface LiquidEtherProps {
  ...
  // Props per simulazione
  simulateTouch?: boolean;
  simulationStart?: SimulationPoint;
  simulationEnd?: SimulationPoint;
  simulationDuration?: number;
  // Nuova prop per disabilitare interazione manuale
  interactive?: boolean;
}
...
export default function LiquidEther({
  ...
  simulateTouch = false,
  simulationStart = { x: 0.2, y: 0.5 },
  simulationEnd = { x: 0.8, y: 0.5 },
  ...
  useEffect(() => {
    ...
    class MouseClass {
      ...
      // NUOVA PROPRIETÀ PER GESTIRE L'INTERATTIVITÀ
      // Se false, ignora tutti gli eventi mouse/touch reali
      ...
      onDocumentMouseMove(event: MouseEvent) {
        // --- BLOCCO TOTALE SE INPUT DISABILITATO ---
    ...
    // Inizializza allowInput con i valori correnti
    Mouse.allowInput = interactive && !simulateTouch;

    interface SimDriverOptions {
      active: boolean;
    ...
    class SimulationDriver {
      ...
      constructor(mouse: MouseClass, opts: SimDriverOptions) {
        this.mouse = mouse;
        this.active = opts.active;
        this.start = opts.start || { x: 0, y: 0 };
        this.end = opts.end || { x: 1, y: 1 };
        this.duration = opts.duration || 2000;
        this.startTime = null;
      }

      updateProps(opts: SimDriverOptions) {
        this.active = opts.active;
      ...
      update() {
        ...
        if (progress > 1) {
          progress = progress % 1;
          this.startTime = now - (elapsed % this.duration);
        }

        const eased = easeInOutCubic(progress);

        const currentX = this.start.x + (this.end.x - this.start.x) * eased;
        const currentY = this.start.y + (this.end.y - this.start.y) * eased;

        const glX = currentX * 2 - 1;
        const glY = -(currentY * 2 - 1);

        this.mouse.isAutoActive = true;
        this.mouse.setNormalized(glX, glY);
      }
    }
    ...
    class AutoDriver {
      ...
      constructor(mouse: MouseClass, manager: any, opts: AutoDriverOptions) {
        this.mouse = mouse;
      ...
      update() {
        ...
        // Disabilita anche l'auto driver randomico se la simulazione guidata è attiva
...
`;
    ...
    class ShaderPass {
      props: any;
      uniforms: { [key: string]: { value: any } } | undefined;
    ...
    class Advection extends ShaderPass {
      ...
      constructor(simProps: any) {
        ...
        if (this.props.material.uniforms) {
            this.uniforms = this.props.material.uniforms;
        }
        this.init();
      }
      ...
      update(params?: { dt: number; isBounce: boolean; BFECC: boolean }) {
        if (!this.uniforms || !this.line || !params) return;
    ...
    class Viscous extends ShaderPass {
      ...
      update(params?: { viscous: number; iterations: number; dt: number }) {
        if (!this.uniforms || !params) return;
        let fbo_in, fbo_out;
    ...
    class Simulation {
      ...
      createShaderPass() {
        if (!this.fbos.vel_0 || !this.fbos.vel_1) return;
    ...
    class WebGLManager {
      props: LiquidEtherProps & { $wrapper: HTMLElement };
      output: Output | null = null;
      lastUserInteraction: number;
      autoDriver: AutoDriver | null = null;
      simulationDriver: SimulationDriver | null = null;
      running: boolean = false;
      _loop: () => void;
      _resize: () => void;
      _onVisibility: () => void;

      constructor(props: LiquidEtherProps & { $wrapper: HTMLElement }) {
        this.props = props;
        ...
        this.simulationDriver = new SimulationDriver(Mouse, {
          active: simulateTouch,
      ...
      render() {
        if (this.simulationDriver && simulateTouch) {
          this.simulationDriver.update();
        } else if (this.autoDriver) {
    ...
    const webgl = new WebGLManager({
      ...
      autoResumeDelay,
      autoRampDuration,
      // Passiamo anche le props non usate nel costruttore ma necessarie per la logica interna
      ...({} as any)
    });
    ...
    simulationEnd,
    simulationDuration,
    interactive // Dipendenza aggiunta
  ]);

  // UseEffect secondario per aggiornamenti "hot"
  useEffect(() => {
    ...
    // Aggiornamento Mouse.allowInput (Hot update)
    if (webgl.autoDriver && webgl.autoDriver.mouse) {
       webgl.autoDriver.mouse.allowInput = interactive && !simulateTouch;
    }
    ...
    if (webgl.autoDriver) {
      ...
      webgl.autoDriver.rampDurationMs = (autoRampDuration || 0) * 1000;
    ...
    if (webgl.simulationDriver) {
      webgl.simulationDriver.updateProps({
        active: simulateTouch,
        start: simulationStart,
        end: simulationEnd,
        duration: simulationDuration
  ...
  return (
    <div
      ref={mountRef}
      className={`relative bg-lotus-blue w-full h-full overflow-hidden touch-none ${className || ''}`}
      style={style}
    />
  );
}